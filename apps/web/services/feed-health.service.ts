...
// Session silence is determined by the market-engine; default false on the Next.js side
...
export class FeedHealthService {
    ...
    assertFeedHealthy(instrumentToken?: string): void {
        ...
        if (this.bootstrapping) {
            ...
            if (this.websocketConnected || elapsed < this.BOOTSTRAP_TIMEOUT_MS) {
                return;
            }
        }
    ...
    private evaluateHealth(): void {
        ...
        const websocketConnected = this.websocketConnected;
    ...
    private getSubscribedTokens(): string[] {
        // Use the set of tokens we have seen prices for as the "subscribed" set.
        // The market-engine owns the actual subscription list.
        return Array.from(this.staleByToken.keys());
    }
}
