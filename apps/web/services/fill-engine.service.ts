...
export class FillEngineService {
    ...
    private static resolveTick(instrument: Instrument): { price: number; timestampMs: number | null; source: TickDataSource } | null {
        ...
        for (const liveQuote of candidates) {
            ...
            // Prefer exchange-provided timestamp (already in ms from ws-server).
            // Fall back to lastUpdated (wall-clock receipt time) only when absent.
            // Using lastUpdated alone is unreliable: reconnects + snapshot seeds make
            // the reception time appear fresh even for stale exchange prices.
            const exchangeTs = Number.isFinite(liveQuote.timestamp) && (liveQuote.timestamp as number) > 0
                ? liveQuote.timestamp as number
                : null;
            const receptionTs = liveQuote.lastUpdated instanceof Date
