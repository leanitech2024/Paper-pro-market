...
export class EventReplayService {
    async rebuildUserState(userId: string, options: ReplayOptions = {}): Promise<RebuiltUserStateSnapshot> {
        ...
        // Check snapshot starting state (from ledger_account_balances)
        const [snapshot] = await db
            .select({
                balance: ledgerAccountBalances.balance,
                lastSequence: ledgerAccountBalances.lastSequence,
            })
            .from(ledgerAccountBalances)
            .innerJoin(
                ledgerAccounts,
                eq(ledgerAccountBalances.accountId, ledgerAccounts.id)
            )
            .where(
                and(
        ...
        while (true) {
            ...
            // Optimization: Skip entries prior to our cached snapshot
            if (lastSequence > 0) {
                whereConditions.push(
                    sql`${ledgerEntries.globalSequence} > ${lastSequence}`
                );
            }
