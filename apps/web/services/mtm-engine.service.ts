...
export class MtmEngineService {
    ...
    private async flushSnapshots(): Promise<void> {
        ...
        try {
            ...
            const updates: Array<{ userId: string; equity: string; marginStatus: string }> = [];
            ...
            // Chunk into batches of 500 to keep query size bounded
            const CHUNK_SIZE = 500;
            for (let i = 0; i < updates.length; i += CHUNK_SIZE) {
                const chunk = updates.slice(i, i + CHUNK_SIZE);

                // Use a transaction with individually parameterized updates rather than
                // sql.raw() string interpolation, which is safe against injection and
                // handles NaN/Infinity values from corrupt state gracefully.
                await db.transaction(async (tx) => {
                    for (const u of chunk) {
                        const equityNum = parseFloat(u.equity);
                        if (!Number.isFinite(equityNum)) continue; // skip corrupt state
                        await tx
                            .update(wallets)
                            .set({
                                equity: equityNum.toFixed(2),
                                marginStatus: u.marginStatus as "NORMAL" | "MARGIN_STRESSED",
                                updatedAt: new Date(),
                            })
            ...
            logger.info({ count: updates.length }, "MTM flush completed (bulk)");
        } catch (error) {
