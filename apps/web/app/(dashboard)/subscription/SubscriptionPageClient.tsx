...
export default function SubscriptionPageClient({ session }: { session: Session | null }) {
  ...
  const [confirmModal, setConfirmModal] = useState<{ isOpen: boolean; targetPlan: "basic" | "pro" | null }>({
    isOpen: false,
    targetPlan: null,
  });
  ...
  const initiateUpgrade = (targetPlan: "basic" | "pro") => {
    ...
    if (plan === "pro" && status === "active" && targetPlan === "basic") {
      toast.info("You already have an active Pro Plan, which includes all Basic features. There is no need to switch!", {
        id: "upgrade-info",
      });
      return;
    }
    ...
    try {
      ...
      const rzpOptions: Record<string, unknown> = {
        ...
        handler: async (response: { razorpay_order_id: string; razorpay_payment_id: string; razorpay_signature: string }) => {
          ...
          setIsUpgrading(null);
          redirectToSuccess(
            `${successUrl}&order_id=${encodeURIComponent(response.razorpay_order_id)}&payment_id=${encodeURIComponent(
  ...
  return (
    <div className="min-h-[calc(100vh-4rem)] p-4 md:p-8 lg:p-12 font-sans">
      <div className="max-w-5xl mx-auto space-y-12">
        {/* Header Section */}
        <div>
          <h1 className="text-3xl font-extrabold tracking-tight text-foreground">Subscription Plans</h1>
        ...
        <section
          className={`rounded-3xl border p-6 flex flex-col sm:flex-row items-center justify-between gap-6 ${
        ...
        <div className="grid md:grid-cols-2 gap-8 w-full mt-4">
          {/* Basic Tier */}
          <div
            className={`relative flex flex-col rounded-3xl p-8 border bg-card/40 backdrop-blur-xl ${
          ...
          >
            <div className="mb-8">
              ...
              <div className="mt-4 flex items-baseline gap-1">
                <span className="text-3xl font-extrabold text-foreground" style={{ fontFamily: "Segoe UI, Noto Sans, Arial, sans-serif" }}>
                  {RUPEE}89
                </span>
        ...
        <Dialog open={confirmModal.isOpen} onOpenChange={(open) => setConfirmModal({ ...confirmModal, isOpen: open })}>
          <DialogContent className="sm:max-w-md">
            ...
            <div className="py-2 text-sm text-foreground border-y border-border/50">
              {plan === "free_trial" && <p>Switching now will immediately end your Free Trial and start your new paid subscription.</p>}
        ...
        <div className="pt-8 text-center sm:text-left">
          <p className="text-xs text-muted-foreground font-medium">
            Need help? Contact{" "}
            <a href="mailto:support@papermarket.in" className="text-primary hover:underline">
              support@papermarket.in
            </a>{" "}
