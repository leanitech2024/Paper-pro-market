...
import { maintenanceMarginService } from "@/domains/trading/server/valuation/maintenance-margin.service";
...
export class LiquidationEngineService {
    ...
    private async liquidationLoop(userId: string, force: boolean): Promise<void> {
        // Load state ONCE before the loop ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â avoids N+1 DB reads during liquidation
        ...
        for (let step = 0; step < MAX_LIQUIDATION_STEPS; step++) {
            ...
            // Check breach using already-loaded risk ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â no DB query per iteration
            const breached =
