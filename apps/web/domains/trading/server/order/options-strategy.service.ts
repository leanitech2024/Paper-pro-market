...
import type { SystemQuoteDetail } from "@/domains/market/server/feeds/upstox-feed.service";
...
export class OptionsStrategyService {
    static async previewStrategy(
        ...
        for (const leg of resolved) {
            ...
            requiredMargin += await MarginCalculatorService.calculateRequiredMargin(
                marginPayload,
                leg.instrument
            );
        }
