...
import { PriceResolverService } from "@/domains/market/server/pricing/price-resolver.service";
...
async function computeRequiredMargin(
    ...
    if (instrumentType === "OPTION") {
        ...
        const underlyingPrice = await resolveOptionUnderlyingPrice(instrumentToken, markPrice);
...
export class PreTradeRiskService {
    ...
    static async validateOrder(
        ...
        const projected = await this.buildProjectedPositions(currentPositions, payload, instrument);
        ...
        const projectedMargins = await Promise.all(
            projected.map((item) =>
        ...
        const projectedRequiredMargin = projectedMargins.reduce((sum, value) => sum + value, 0);
    ...
    private static async buildProjectedPositions(
        ...
        const referencePrice = await this.resolveReferencePrice(payload, instrument);
    ...
    private static async resolveReferencePrice(payload: PlaceOrder, instrument: Instrument): Promise<number> {
