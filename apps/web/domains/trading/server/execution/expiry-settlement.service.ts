...
import { liquidationEngineService } from "@/domains/trading/server/valuation/liquidation-engine.service";
...
export class ExpirySettlementService {
    ...
    async settleUserPosition(
        ...
        // C-6 FIX: Include userId in the idempotency key. Without it, all users
        // holding the same expired instrument on the same date share one key,
        // causing cross-user deduplication collisions.
        const idempotencyKey = `SETTLEMENT-${position.userId}-${position.instrumentToken}-${position.settlementDateKey}`;
