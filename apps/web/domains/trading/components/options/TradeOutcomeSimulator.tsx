...
import { calculateLegPnL } from "@/domains/trading/lib/options/multi-leg-payoff";
import type { PayoffOptionType, PayoffSide } from "@/domains/trading/lib/options/multi-leg-payoff";
...
function formatMoney(value: number): string {
  ...
  const sign = value >= 0 ? "+" : "-";
  return `${sign}â‚¹${abs.toLocaleString("en-IN", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`;
}

const RANGE_PCT = 0.07; // Â±7% from spot
...
export function TradeOutcomeSimulator({
  ...
  const pctFill = useMemo(() => {
    // Normalize slider to 0â€“100 for the fill bar
  ...
  return (
    <div className="rounded-lg border border-white/[0.08] bg-white/[0.02] p-3 space-y-3">
      <div className="flex items-center justify-between">
        <p className="text-xs font-semibold text-slate-200">What Happens Ifâ€¦</p>
      ...
      <div className="flex items-center justify-between rounded-md bg-white/[0.04] px-3 py-2">
        <div className="text-xs text-slate-400">
          ...
          <span className="font-medium text-white">
            â‚¹{simulatedSpot.toLocaleString("en-IN", { maximumFractionDigits: 0 })}
          </span>
        </div>
        <div
          className={cn(
