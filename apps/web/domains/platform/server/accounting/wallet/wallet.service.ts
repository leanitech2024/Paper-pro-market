...
export class WalletService {
    ...
    static async getAvailableBalance(userId: string): Promise<number> {
        // Read from ledger (source of truth) â€” bypasses stale wallet cache
        ...
        const apply = async (executor: TxLike): Promise<void> => {
            ...
            try {
                ...
                if (shouldJournal && preparedJournalId) {
                    // C-5 FIX: Use undefined (global db) not executor â€” if executor
    ...
    static async debitBalance(
        ...
        // For P&L settlement debits, blocked margin already covers the loss â€” skip cash check
    ...
    static async getTransactions(
        ...
        // ledger load was O(all entries) in memory â€” lethal for power users.
