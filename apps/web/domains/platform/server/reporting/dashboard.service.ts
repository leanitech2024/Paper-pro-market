...
import { instruments, orders, positions, transactions } from "@paper-market/core/db";
import {
...
export class DashboardService {
  static async getOverview(userId: string): Promise<DashboardOverviewData> {
    ...
    // H-4 FIX: Aggregate P&L directly in SQL instead of fetching all closed order rows
    ...
    const [pnlAgg] = await db
      .select({
        ...
        closedTradeCount: sql<number>`COUNT(CASE WHEN ${orders.realizedPnL} IS NOT NULL THEN 1 END)`,
        winningTradeCount: sql<number>`COUNT(CASE WHEN ${orders.realizedPnL}::numeric > 0 THEN 1 END)`,
        bestTrade: sql<number>`COALESCE(MAX(CASE WHEN ${orders.realizedPnL} IS NOT NULL THEN ${orders.realizedPnL}::numeric END), 0)`,
        dailyPnL: sql<number>`COALESCE(SUM(CASE
          WHEN ${orders.realizedPnL} IS NOT NULL
            AND COALESCE(${orders.executedAt}, ${orders.updatedAt}, ${orders.createdAt}) >= ${dayStartUtc.toISOString()}::timestamptz
            AND COALESCE(${orders.executedAt}, ${orders.updatedAt}, ${orders.createdAt}) < ${dayEndUtc.toISOString()}::timestamptz
          THEN ${orders.realizedPnL}::numeric ELSE 0 END), 0)`,
      })
      .from(orders)
      .where(eq(orders.userId, userId));

    const closedPnL = toFiniteNumber(pnlAgg?.closedPnL);
    const closedTradeCount = Number(pnlAgg?.closedTradeCount ?? 0);
    const winningTradeCount = Number(pnlAgg?.winningTradeCount ?? 0);
    const bestTrade = toFiniteNumber(pnlAgg?.bestTrade);
    const dailyPnL = toFiniteNumber(pnlAgg?.dailyPnL);

    // H-5 FIX: Cap equity curve to the 500 most recent transactions.
    // Previously fetched ALL transactions sorted by date â€” for active traders
    // this could be thousands of rows loaded into memory on every dashboard request.
    ...
    // Equity curve is fetched descending for recency cap â€” reverse for chronological charts
    const equityCurve =
      equityRows.length > 0
        ? equityRows.reverse().map((row) => ({
            time:
