        ...
        <div className="flex flex-col">
          {localMatches.map((stock, i) => {
            const quoteKey = stock.instrumentToken
              ...
              : "";
            const symbolFallbackKey = stock.symbol
              ? (() => {
                  if (quoteKey && quoteKey.includes("|")) {
                    const prefix = quoteKey.split("|")[0] || "";
                    const sym = toCanonicalSymbol(stock.symbol);
                    return prefix && sym ? `${prefix}|${sym}` : "";
                  }
                  return toInstrumentKey(stock.symbol);
                })()
              : "";
            const quote =
              (quoteKey ? quotesByInstrument[quoteKey] : null) ||
              (symbolFallbackKey ? quotesByInstrument[symbolFallbackKey] : null) ||
              (quoteKey ? selectQuote(quoteKey) : null) ||
              (symbolFallbackKey ? selectQuote(symbolFallbackKey) : null) ||
              (stock.symbol ? selectQuote(stock.symbol) : null);
            ...
            const fallbackClose = hasFallbackPrice
              ? (Number.isFinite(fallbackChange)
                  ? fallbackPrice - fallbackChange
                  : (Number.isFinite(fallbackChangePercent) && (1 + fallbackChangePercent / 100) !== 0
                      ? fallbackPrice / (1 + fallbackChangePercent / 100)
                      : undefined))
              : undefined;
            const liveClose = Number.isFinite(quoteClose) && (quoteClose ?? 0) > 0
              ? quoteClose
              : (Number.isFinite(fallbackClose) && (fallbackClose ?? 0) > 0 ? fallbackClose : undefined);

            const liveChange = Number.isFinite(livePrice) && (livePrice ?? 0) > 0 && Number.isFinite(liveClose) && (liveClose ?? 0) > 0
              ? (livePrice as number) - (liveClose as number)
              : (hasFallbackPrice && Number.isFinite(fallbackChange) ? fallbackChange : undefined);
            const liveChangePercent = Number.isFinite(livePrice) && (livePrice ?? 0) > 0 && Number.isFinite(liveClose) && (liveClose ?? 0) > 0
              ? ((liveChange as number) / (liveClose as number)) * 100
            ...
            <div
              ...
              className={cn(
                "group flex items-center justify-between px-3 py-2.5 border-b border-slate-200/60 dark:border-white/[0.05] cursor-pointer transition-colors hover:bg-slate-100/50 dark:hover:bg-slate-800/40",
                selectedSymbolKey === toSymbolKey(toCanonicalSymbol(stock.symbol)) &&
                  "bg-slate-200/60 border-l-2 border-l-primary dark:bg-slate-800/80"
              )}
            >
              {/* Left: Symbol + Name */}
