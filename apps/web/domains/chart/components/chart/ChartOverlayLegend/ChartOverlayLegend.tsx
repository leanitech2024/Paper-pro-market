import { formatPrice, formatVolume, calculateChange } from './utils';
...
export function ChartOverlayLegend({
  ...
  const { change, changePct } = calculateChange(data.open, data.close);
  const volumeString = formatVolume(data.volume);
    ...
    <div className="absolute top-2 left-2 z-20 select-none flex flex-col gap-1.5 pointer-events-none">
      ...
      <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-[10px] md:text-xs font-mono max-w-[90vw]">
        <div className="flex gap-1">
          ...
          <span style={{ color: valueColor }}>{formatPrice(data.open)}</span>
        </div>
        <div className="flex gap-1">
          <span className="text-muted-foreground">H</span>
          <span style={{ color: valueColor }}>{formatPrice(data.high)}</span>
        </div>
        <div className="flex gap-1">
          <span className="text-muted-foreground">L</span>
          <span style={{ color: valueColor }}>{formatPrice(data.low)}</span>
        </div>
        <div className="flex gap-1">
          <span className="text-muted-foreground">C</span>
          <span style={{ color: valueColor }}>{formatPrice(data.close)}</span>
        </div>
        <div className="basis-full h-0 m-0"></div> {/* Line break on mobile for volume/change */}
        {volumeString && (
          <div className="flex gap-1">
            <span className="text-muted-foreground">Vol</span>
            <span className="text-foreground">{volumeString}</span>
          </div>
        )}
        <div className="flex gap-1">
