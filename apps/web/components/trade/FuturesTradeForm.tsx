...
export function FuturesTradeForm({ selectedStock, onStockSelect, instruments, onOpenSearch, isBootstrapping = false, sheetMode = false }: FuturesTradeFormProps) {
  ...
  useEffect(() => { fetchWallet().catch(() => undefined); }, [fetchWallet]);
  useEffect(() => { fetchPositions(true).catch(() => undefined); }, [fetchPositions]);

  // Reset leverage to 1 whenever product type switches to CNC
  useEffect(() => { if (productType === "CNC") setLeverage("1"); }, [productType]);

  const availableExpiries = useMemo(() => {
    const dates = instruments.map((item) => parseExpiryDate(item.expiryDate)).filter((d): d is Date => Boolean(d));
    return Array.from(new Set(dates.map((d) => d.getTime()))).map((t) => new Date(t)).sort((a, b) => a.getTime() - b.getTime());
  }, [instruments]);

  useEffect(() => {
    if (!selectedStock) { setSelectedExpiry(""); return; }
    const currentExpiry = toExpiryIso(selectedStock.expiryDate);
    if (currentExpiry) { setSelectedExpiry((prev) => (prev === currentExpiry ? prev : currentExpiry)); return; }
    if (availableExpiries.length > 0) {
      const fallback = availableExpiries[0].toISOString();
      setSelectedExpiry((prev) => (prev === fallback ? prev : fallback));
    }
  ...
  useEffect(() => {
    ...
    const candidates = instruments.filter(
      (item) => normalizeUnderlyingKey(item.name) === selectedUnderlying && toExpiryIso(item.expiryDate) === selectedExpiry
    );
    if (candidates.length === 0) return;
    const match = candidates.find((item) => item.symbol === selectedStock.symbol) ?? candidates[0];
    if (match.instrumentToken !== selectedStock.instrumentToken) onStockSelect(match);
  }, [instruments, onStockSelect, selectedExpiry, selectedStock]);

  const currentPrice = liveTokenPrice || chartLastPrice || liveSymbolPrice || liveUnderlyingPrice || selectedStock?.price || 0;
  ...
  const existingPosition = useMemo(() => {
    ...
    return positions.find((p) => String(p.instrumentToken || "") === token && Number(p.quantity || 0) > 0) || null;
  ...
  const requiredMargin = calculateFuturesRequiredMargin({
    ...
    instrument: { underlying: selectedStock?.name, name: selectedStock?.name, tradingsymbol: selectedStock?.symbol, symbol: selectedStock?.symbol },
  });
  ...
  const canTrade = Boolean(selectedStock) && hasToken && hasValidPrice && !isOrderProcessing && isQuantityValid && hasSufficientMargin && isSlValid && isTargetValid;

  const handleSubmit = () => {
    if (isOrderProcessing || !selectedStock || !canTrade) return;
    if (!selectedStock.instrumentToken) { toast.error("Instrument routing key missing"); return; }
  ...
  const confirmTrade = async () => {
    ...
    try {
      // ── FIX: productType, stopLossPrice, targetPrice now flow through ──────
      await executeTrade(
        {
          ...
          leverage: leverageValue,
          productType,                                         // ← was missing
          stopLossPrice: hasSl ? slValue : undefined,          // ← was missing
          targetPrice: hasTarget ? targetValue : undefined,    // ← was missing
        },
        lotSize,
        "futures"
      );

      toast.success("Trade Sent", { description: `${side} ${effectiveQuantity} ${selectedStock.symbol}` });
  ...
  return (
    <div className={cn("flex h-full min-h-0 flex-col bg-[#0f1728]", sheetMode && "rounded-none")}>
      <div className="flex shrink-0 items-center justify-between border-b border-slate-200/80 dark:border-white/[0.08] px-4 py-3">
        <div className="text-[11px] font-semibold uppercase tracking-[0.12em] text-muted-foreground">Futures Order</div>
        <button type="button" onClick={onOpenSearch} className="inline-flex items-center gap-1.5 rounded-md border border-slate-200/80 dark:border-white/[0.08] bg-white/40 dark:bg-white/5 px-2 py-1 text-[10px] font-semibold text-foreground hover:bg-slate-100 dark:hover:bg-white/10">
          <Search className="h-3 w-3" />Search
        </button>
      </div>

      <div className={cn("flex-1 min-h-0 p-0", sheetMode && "pb-20")}>
        <ScrollArea className="h-full w-full overscroll-y-contain">
            ...
            {!selectedStock ? (
              <div className="flex h-full min-h-[220px] items-center justify-center rounded-xl border border-slate-200/80 dark:border-white/[0.08] bg-white/40 dark:bg-white/[0.02]">
                {isBootstrapping ? (
                  ...
                  <div className="text-center">
                    <p className="text-sm font-semibold text-foreground">No contract selected</p>
                    <p className="mt-1 text-xs text-muted-foreground">Search and select a futures contract.</p>
                  </div>
                )}
              </div>
            ) : (
              <>
                <div className="space-y-3 rounded-xl border border-slate-200/80 dark:border-white/[0.08] bg-white/40 dark:bg-white/[0.02] p-3">
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0">
                      <p className="text-[10px] uppercase tracking-wider text-muted-foreground">Selected Contract</p>
                      <p className="mt-1 text-sm font-bold text-foreground">{selectedStock.symbol}</p>
                    </div>
                  </div>
                  {availableExpiries.length > 0 && (
                    <div className="space-y-1.5">
                      ...
                      <Select value={selectedExpiry} onValueChange={setSelectedExpiry}>
                        <SelectTrigger className="h-8 rounded-md border-slate-200/80 dark:border-white/[0.08] bg-white/40 dark:bg-white/[0.02] text-xs"><SelectValue placeholder="Select Expiry" /></SelectTrigger>
                ...
                <div className="rounded-xl border border-slate-200/80 dark:border-white/[0.08] bg-white/40 dark:bg-white/[0.02] px-3 py-2.5">
                  ...
                  <div className="mt-1 flex items-center justify-between">
                    ...
                    <span className={cn("text-xs font-semibold", side === "BUY" ? "text-emerald-500" : "text-rose-500")}>{side}</span>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <p className="mb-1 text-[10px] uppercase tracking-wider text-muted-foreground">Lots</p>
                    {isOppositeExitFlow ? (
                      <div className="flex h-9 items-center rounded-md border border-slate-200/80 dark:border-white/[0.08] bg-white/40 dark:bg-white/[0.02] px-3 text-sm font-semibold text-foreground">Exit ({existingPositionQty})</div>
                    ) : (
                      <div className="flex items-center overflow-hidden rounded-md border border-slate-200/80 dark:border-white/[0.08] bg-white/40 dark:bg-white/[0.02]">
                        <button type="button" onClick={() => setQuantity(String(Math.max(1, inputLots - 1)))} className="px-3 py-2 text-muted-foreground hover:text-foreground">-</button>
                        ...
                        <button type="button" onClick={() => setQuantity(String(inputLots + 1))} className="px-3 py-2 text-muted-foreground hover:text-foreground">+</button>
                      </div>
                    )}
                  </div>
                  <div>
                    <p className="mb-1 text-[10px] uppercase tracking-wider text-muted-foreground">Quantity</p>
                    <div className="flex h-9 items-center rounded-md border border-slate-200/80 dark:border-white/[0.08] bg-white/40 dark:bg-white/[0.02] px-3 text-sm font-bold tabular-nums text-foreground">{effectiveQuantity.toLocaleString("en-IN")}</div>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <div className="inline-flex w-full rounded-md border border-slate-200/80 dark:border-white/[0.08] bg-white/40 dark:bg-white/[0.02] p-0.5">
                      {(["CNC", "MIS"] as const).map((value) => (
                        <button key={value} type="button" onClick={() => setProductType(value)}
                          className={cn("w-1/2 rounded-sm py-1.5 text-[11px] font-semibold",
                            productType === value ? "bg-white dark:bg-white/10 shadow-sm text-foreground" : "text-muted-foreground hover:text-foreground")}>
                          {value}
                        </button>
                      ))}
                    </div>
                  ...
                  <div>
                    <p className="mb-1 text-[10px] uppercase tracking-wider text-muted-foreground">
                      Leverage {productType === 'CNC' && <span className="text-[9px]">(CNC=1x)</span>}
                    </p>
                    <div className="inline-flex w-full rounded-md border border-slate-200/80 dark:border-white/[0.08] bg-white/40 dark:bg-white/[0.02] p-0.5">
                      {["1", "2", "3", "5"].map((value) => (
                        <button key={value} type="button"
                          onClick={() => productType === 'MIS' && setLeverage(value)}
                          disabled={productType === 'CNC'}
                          className={cn("w-1/4 rounded-sm py-1.5 text-[11px] font-semibold",
                            leverage === value ? "bg-white dark:bg-white/10 shadow-sm text-foreground" : "text-muted-foreground hover:text-foreground",
                            productType === 'CNC' && "opacity-40 cursor-not-allowed")}>
                          {value}x
                        </button>
                      ))}
                    </div>
                ...
                <div className="grid grid-cols-2 gap-3 pb-2">
                  <div>
                    ...
                    <input type="number" placeholder={side === "BUY" ? "< Entry" : "> Entry"} value={stopLoss} onChange={(e) => setStopLoss(e.target.value)}
                      className={cn("h-9 w-full rounded-md border bg-white/40 dark:bg-white/[0.02] px-3 text-sm font-mono text-foreground placeholder:text-muted-foreground outline-none",
                        hasSl && !isSlValid ? "border-rose-500/60 focus:ring-1 focus:ring-rose-500/40" : "border-slate-200/80 dark:border-white/[0.08] focus:ring-1 focus:ring-primary/40")} />
                  </div>
                  <div>
                    ...
                    <input type="number" placeholder={side === "BUY" ? "> Entry" : "< Entry"} value={target} onChange={(e) => setTarget(e.target.value)}
                      className={cn("h-9 w-full rounded-md border bg-white/40 dark:bg-white/[0.02] px-3 text-sm font-mono text-foreground placeholder:text-muted-foreground outline-none",
                        hasTarget && !isTargetValid ? "border-rose-500/60 focus:ring-1 focus:ring-rose-500/40" : "border-slate-200/80 dark:border-white/[0.08] focus:ring-1 focus:ring-primary/40")} />
                  </div>
                </div>
                ...
                <div className={cn("pt-2", sheetMode && "sticky bottom-0 bg-card py-3")}>
      ...
      <TradeConfirmationDialog open={showConfirmDialog} onOpenChange={setShowConfirmDialog} selectedStock={selectedStock} side={side} quantityValue={effectiveQuantity} currentPrice={currentPrice} requiredMargin={requiredMargin} productType={productType} leverageValue={leverageValue} isProcessing={isOrderProcessing} onConfirm={confirmTrade} />
    </div>
  );
}
