...
export function isInstrumentAllowed(instrument: {
  ...
  // 1. Check Exchange
  if (!(TRADING_UNIVERSE.exchanges as readonly string[]).includes(instrument.exchange)) {
      return { allowed: false, reason: `Exchange ${instrument.exchange} is not in trading universe` };
  }

  // 2. Check Segment
  if (!(TRADING_UNIVERSE.segments as readonly string[]).includes(instrument.segment)) {
