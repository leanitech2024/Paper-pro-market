...
export function isIndexFutureInstrument(instrument) {
    if (!instrument)
        return false;
    const candidates = [
...
export function resolveFuturesMarginPercent(instrument) {
    return isIndexFutureInstrument(instrument)
        ? INDEX_FUTURES_MARGIN_PERCENT
        : STOCK_FUTURES_MARGIN_PERCENT;
}
export function resolveEffectiveLeverage(leverage) {
