...
export function calculateAnnualizedSharpeRatioFromEquityCurve(equityCurve) {
    if (!Array.isArray(equityCurve) || equityCurve.length < 2)
        return 0;
    const ordered = [...equityCurve].sort((a, b) => a.time - b.time);
    const dailyCloseByKey = new Map();
    for (const point of ordered) {
        const value = Number(point.value);
        if (!Number.isFinite(value) || value <= 0)
            continue;
...
export function getIstDayBoundsUtc(now = new Date()) {
