...
function extractUnderlyingHints(tradingsymbol) {
...
function matchesUniverseCandidate(candidateKeys, allowedSet, allowedPrefixKeys) {
...
export function isInstrumentAllowed(instrument) {
    // 1. Check Exchange
    if (!TRADING_UNIVERSE.exchanges.includes(instrument.exchange)) {
        return { allowed: false, reason: `Exchange ${instrument.exchange} is not in trading universe` };
    }
    // 2. Check Segment
    if (!TRADING_UNIVERSE.segments.includes(instrument.segment)) {
