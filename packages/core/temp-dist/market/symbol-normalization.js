...
export function toCanonicalSymbol(symbol) {
 ...
 */
export function toInstrumentKey(value) {
    if (!value)
        return "";
...
export function symbolToIndexInstrumentKey(symbol) {
    const canonical = toCanonicalSymbol(symbol);
    if (!canonical)
        return null;
    // Use toInstrumentKey to ensure proper format (mixed case for indices)
    if (canonical === "NIFTY 50")
        return toInstrumentKey("NSE_INDEX|Nifty 50");
    if (canonical === "NIFTY BANK")
        return toInstrumentKey("NSE_INDEX|Nifty Bank");
    if (canonical === "NIFTY FIN SERVICE")
        return toInstrumentKey("NSE_INDEX|Nifty Fin Service");
    return null;
}
