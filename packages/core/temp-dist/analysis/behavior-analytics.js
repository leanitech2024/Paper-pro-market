 ...
 */
function detectHighFrequency(entries) {
    const insights = [];
    if (entries.length < CONFIG.HIGH_FREQ_COUNT)
        return insights;
    // Sort by entry time
 ...
 */
function detectRapidReentry(entries) {
    const insights = [];
 ...
 */
function detectLossStreaks(entries) {
    const insights = [];
 ...
 */
function detectHoldingLosers(entries) {
    const insights = [];
    const closed = entries.filter(e => e.exitTime && e.realizedPnL !== undefined);
